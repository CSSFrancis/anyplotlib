...
class TestPanelCrop:
    ...
    @pytest.mark.parametrize("bad", ["lite", "Dark", "auto", "light "])
    def test_unknown_theme_is_rejected(self, mount_page, bad):
        """Falling back to the ambient theme would leave a host believing it had
        asked for a light export and getting a dark one, with nothing to see."""
        fig, ax = apl.subplots(1, 1, figsize=(FIGW, FIGH))
        ax.imshow(_ramp(), tile=False)
        page = mount_page(fig)
        res = export_via_handle(page, {"theme": bad})
        assert "error" in res, f"theme={bad!r} was accepted silently"
        assert "theme must be" in res["error"], res["error"]
