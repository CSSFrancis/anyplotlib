...
class TestContextMenu:
        ...
        page.wait_for_function(
            """() => {
                 const e = document.querySelector(
                   "div[title='Copy or save this figure']");
    ...
    def test_badge_is_reachable_by_keyboard(self, mount_page):
        """The badge is hidden until the pointer moves, and a keyboard-only user
        never moves one — so focusing it must reveal it, and Enter/Space must
        activate it."""
        fig, plot = _fig()
        page = mount_page(fig)
        badge = page.locator("div[title='Copy or save this figure']")

        assert badge.get_attribute("role") == "button"
        assert badge.get_attribute("aria-label")
        assert badge.evaluate("e => e.tabIndex") == 0

        badge.evaluate("e => e.focus()")
        # 120 ms opacity transition — poll rather than sample mid-animation.
        page.wait_for_function(
            """() => {
                 const e = document.querySelector(
                   "div[title='Copy or save this figure']");
