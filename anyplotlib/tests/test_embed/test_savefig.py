...
class TestSavefigNative:
    ...
    def test_native_respects_origin_lower(self, tmp_path):
        """Backend rows are in SOURCE order, so an origin='lower' plot must be
        flipped to display order for the export — the same correction the
        overview and the detail tiles apply."""
        n = 1200
        data = np.zeros((n, n), dtype=np.float32)
        data[: n // 2] = 255.0            # source TOP half bright
        fig, ax = apl.subplots(1, 1, figsize=(FIGW, FIGH))
        plot = ax.imshow(data, cmap="gray", vmin=0, vmax=255, origin="lower")
        assert plot.to_state_dict()["tile_enabled"]

        arr = _read(fig.savefig(tmp_path / "lower.png", source="native", panel=plot))
        img = arr[-n:, :n, 0].astype(int)   # the image area, below the title strip
        # origin='lower' puts source row 0 at the BOTTOM, so the bright source
        # top half must render in the upper half of the output.
        assert img[10].mean() < 40, f"top row mean {img[10].mean():.0f}, expected dark"
        assert img[-10].mean() > 215, f"bottom row mean {img[-10].mean():.0f}, expected bright"

    def test_panel_from_another_figure_is_rejected(self, tmp_path):
        fig, ax = apl.subplots(1, 1, figsize=(FIGW, FIGH))
        ax.imshow(_ramp(32), tile=False)
        other, oax = apl.subplots(1, 1, figsize=(FIGW, FIGH))
        stranger = oax.imshow(_ramp(32), tile=False)

        with pytest.raises(ValueError, match="not a plot of this figure"):
            fig.savefig(tmp_path / "x.png", panel=stranger)

    def test_native_on_a_named_non_2d_panel_is_rejected(self, tmp_path):
        fig, axes = apl.subplots(1, 2, figsize=(520, 240))
        axes[0].imshow(_ramp(32), tile=False)
        line = axes[1].plot(np.arange(10, dtype=float))

        with pytest.raises(ValueError, match="2-D image panels"):
            fig.savefig(tmp_path / "x.png", source="native", panel=line)

    def test_a_malformed_export_payload_raises(self, tmp_path, monkeypatch):
        """The page is supposed to hand back a base64 PNG data URL; anything
        else must fail loudly rather than write a corrupt file."""
        import anyplotlib._export as ex

        monkeypatch.setattr(ex, "run_in_page",
                            lambda *a, **k: "data:text/plain;base64,Zm9v")
        fig, ax = apl.subplots(1, 1, figsize=(FIGW, FIGH))
        ax.imshow(_ramp(32), tile=False)
        out = tmp_path / "x.png"
        with pytest.raises(RuntimeError, match="unexpected export payload"):
            fig.savefig(out)
        assert not out.exists(), "a corrupt file was written before the check"
